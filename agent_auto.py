...
#from evaluator_auto import get_score,policy_value_fn
from evaluator_auto_2 import get_score,policy_value_fn
...
class MCTS(object):
    ...
    def __init__(self, df_name,total_time=100):
    ...
    def _playout(self):
        ...
        if not end:
            ...
            leaf_value = get_score(node,self._df_name)
            if leaf_value == -1:
    ...
    def _palyouts(self):
        ...
        while True:
            if time.time() > end_time:
                ...
                #save_model(self._df_name[23:][:-4],best_param,best_score)
                break
            else:
                n = int((time.time() - start_time) // 60)
                if n > k:
                    k= n
                    print('{}th min, best score is {}'.format(k,self.find_intermediate()))
                    #save_score(self._df_name[23:][:-4],k,self.find_intermediate())
                self._playout()
...
class Agent():
    ...
    def __init__(self, df_name,total_time=100):
        self.mcts = MCTS(df_name,total_time)
